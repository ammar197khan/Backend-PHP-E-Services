...
@section('content')
    ...
    <ul class="breadcrumb">
        ...
        <li class="active">{{ __('language.Admins') }}</li>
    </ul>
    <!-- END BREADCRUMB -->
                ...
                <div class="panel panel-default">
                    @if(company()->hasPermissionTo('Add admin'))
                        <div class="panel-heading">
                            <a href="{{route('company.admins.create')}}"><button type="button" class="btn btn-info"><i class="fa fa-plus"></i>{{ __('language.+New admin') }} </button></a>
                    ...
                    @endif
                    <form class="form-horizontal" method="get" action="{{route('company.admins.search')}}">
                            ...
                            <table class="table">
                                ...
                                @foreach($admins as $admin)
                                    <tr>
                                        @php
                                        $sub_comp_name = "N/A";
                                        $subCompanyID = $admin->sub_company_id;
                                        if($subCompanyID){
                                        $findSub = DB::table('sub_companies')->where('id', $subCompanyID)->first();
                                        if($sub_comp_name && $findSub != ""){ $sub_comp_name = $findSub->en_name; }}
                                        @endphp
                                        <td>{{$admin->name}}</td>
                                        <td>{{$sub_comp_name}}</td>
                                        <td>{{$admin->email}}</td>
                                        ...
                                        <td>
                                            <a title="View" href="{{route('company.admins.view',$admin->id)}}"><button class="btn btn-info btn-condensed"><i class="fa fa-eye"></i></button></a>
                                            @if(company()->hasPermissionTo('Edit admin'))
                                                <a title="Edit" href="{{route('company.admins.edit',$admin->id)}}"><button class="btn btn-warning btn-condensed"><i class="fa fa-edit"></i></button></a>
                                            @endif
                                            @if($admin->active == 1)
                                                @if(company()->hasPermissionTo('Suspend admin'))
                                                ...
                                                @endif
                                            @else
                                                @if(company()->hasPermissionTo('Active admin'))
                                        ...
                                        <div class="mb-container">
                                            <div class="mb-middle warning-msg alert-msg">
                                                ...
                                                <div class="mb-footer buttons">
                                                    ...
                                                    <form method="post" action="{{route('company.admins.change_status')}}" class="buttons">
                                                        {{csrf_field()}}
